"""Aggregate + plot the scan-type ablation sweep (docs/run_scan_ablation.sh).

Sibling of docs/plot_scorer_ablation.py, but for the *scan-curve* front instead of the
score-readout front. Unlike the scorer sweep (which re-scores ONE trained checkpoint many
ways), scan_type is a decoder-architecture choice baked in at training time, so each curve is
a SEPARATE e50 x log-Mel MambaAD training run. Every run is then re-scored with the SAME
test-time readout — student-maha (the winning readout from the scorer/ front) — so the only
thing that varies across the lines is the Mamba scan order.

Reads one 42-column metric file per scan type (produced by docs/reeval_sp_mean.py re-scoring
each run at every saved epoch under the student-maha scorer) and emits, for each figure, a
self-contained folder holding BOTH the PNG and the CSV of the numbers behind it:

    docs/plots/mimii_scan/
        scan_summary.csv                          # master table (all scans/classes/metrics)
...
student-maha is an image-level readout (broadcast constant map), so its sp_max == sp_mean by
construction — the two families are identical here; both are emitted for layout parity with the
scorer sweep. Column layout of each metric file matches trainer/_base_trainer.py:_finish (see
...
Avg interleaved after the last class. Rows are epoch-aligned; only saved-epoch rows are nonzero.
"""
import argparse
import csv
import os
...
# Scan curves, in legend order. style: (color, linestyle). hilbert is the baseline (bold black);
# the two locality-preserving curves (hilbert, zorder) are solid, the raster-family ones dashed.
SCANS = {
    "hilbert": dict(color="#111111", ls="-",  label="hilbert (baseline)"),
    "zorder":  dict(color="#1f77b4", ls="-",  label="zorder"),
    "scan":    dict(color="#2ca02c", ls="--", label="scan (serpentine)"),
    "sweep":   dict(color="#d62728", ls="--", label="sweep (raster)"),
    "zigzag":  dict(color="#ff7f0e", ls="--", label="zigzag"),
}
...
OUT_ROOT = "docs/plots/mimii_scan"
...
def main():
    ...
    ap.add_argument("--data-dir", default=os.path.join(OUT_ROOT, "_data"),
                    help="dir holding <scan>.txt metric files")
    args = ap.parse_args()
    ...
    for key in SCANS:
        ...
        if not os.path.exists(p):
            ...
            continue
        data[key] = load_scan(p)
    if not data:
        raise SystemExit(f"no scan metric files found in {args.data_dir}")
    ...
    with open(os.path.join(OUT_ROOT, "scan_summary.csv"), "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["scan", "class", "family",
    ...
    print("wrote", os.path.join(OUT_ROOT, "scan_summary.csv"))

    # ================= per-family figures =================
    for fam_name, fam in FAMILIES.items():
        ai = fam["auroc"]

        # ---- Fig A: Avg AUROC vs epoch, one line per scan (peak ring) ----
        ...
        ax.set_title(f"MambaAD / MIMII e50 log-Mel — scan-type ablation (student-maha readout)\n"
        ...
        for ax in axes[-1]:
            ax.set_xlabel("epoch")
        fig.suptitle(f"MambaAD / MIMII e50 log-Mel — per-class AUROC vs. epoch "
                     f"(scan-type ablation, student-maha, {fam_name})", fontsize=13)
        fig.tight_layout(rect=(0, 0, 1, 0.97))
        emit(f"auroc_per_class_vs_epoch_{fam_name}", fig,
             ["class", "scan", "epoch", "AUROC"], rows_csv)

        # ---- Fig C: best-epoch per-class AUROC, grouped bars ----
        # each scan at its own Avg-AUROC peak epoch (fair single-number comparison)
        ...
        ax.set_title(f"MambaAD / MIMII e50 log-Mel — per-class AUROC at each scan's peak "
                     f"(student-maha, {fam_name})")
        ax.grid(alpha=0.3, axis="y")
        ax.legend(fontsize=8, ncol=2)
        fig.tight_layout()
        emit(f"best_per_class_{fam_name}", fig,
             ["scan", "peak_epoch", "class", "AUROC"], bar_rows)

    # ---- console recap ----
    print("\n==> scan recap (Avg AUROC, sp_max family):")
    for key, (ep, vals, col) in data.items():
        y = vals[:, col[("Avg", FAMILIES["sp_max"]["auroc"])]]
        print(f"   {key:8s} start(ep{ep[0]})={y[0]:6.2f}  peak={y.max():6.2f}@ep{ep[int(np.argmax(y))]:<3d}  final={y[-1]:6.2f}")

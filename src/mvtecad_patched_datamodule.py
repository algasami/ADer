...
from mvtecad_patched_dataset import MVTecADPatchedDataset

logger = logging.getLogger(__name__)


class MVTecADPatched(AnomalibDataModule):
    """MVTec AD Datamodule.

    Args:
        root (Path | str | None): Path to the root of the dataset.
            Defaults to ``"./datasets/MVTecAD"``.
        categories (Sequence[str]): Categories of the MVTec AD dataset (e.g. ``("bottle",)`` or
    ...
    def _setup(self, _stage: str | None = None) -> None:
        ...
        """
        self.train_data = MVTecADPatchedDataset(
            split=Split.TRAIN,
            root=self.root,
            categories=self.categories,
        )
        self.test_data = MVTecADPatchedDataset(
            split=Split.TEST,
            root=self.root,
            categories=self.categories,
        )
    ...
    def prepare_data(self) -> None:
        ...
        if (self.root / self.category).is_dir():
            ...
            logger.warning(f"Dataset not found at {self.root}.")

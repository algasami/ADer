...
class MVTecADPatchedDataset(AnomalibDataset):
    """MVTec AD dataset class.

    Dataset class for loading and processing MVTec AD dataset images. Supports
    both classification and segmentation tasks. **Normally MVTecAD only supports
    training on one category at a time, but this patched version allows loading multiple categories
    simultaneously.**

    Args:
        root (Path | str | None): Path to root directory containing the dataset.
            Defaults to ``None``.
        categories (Sequence[str]): Category names, must be a sequence of strings from ``CATEGORIES``.
            Defaults to ``("bottle",)``.
    ...
    Example:
        >>> from pathlib import Path
        >>> from anomalib.data.datasets import MVTecADPatchedDataset
        >>> dataset = MVTecADPatchedDataset(
    ...
    def __init__(
        ...
        categories: Sequence[str] = ("bottle",),
...
def make_mvtec_ad_dataset(
    ...
    samples_list = [(str(root), *f.parts[-4:]) for f in root.glob(r"**/*") if f.suffix in extensions and f.parts[-4] in categories]
    if not samples_list:
        msg = f"Found 0 images in {root}"
        raise RuntimeError(msg)

    samples = DataFrame(samples_list, columns=["path", "category", "split", "label", "image_path"])

    # Modify image_path column by converting to absolute path
    samples["image_path"] = samples.path + "/" + samples.category + "/" + samples.split + "/" + samples.label + "/" + samples.image_path
